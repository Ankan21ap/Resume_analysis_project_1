...
const App: React.FC = () => {
  ...
  const [currentAnalysis, setCurrentAnalysis] = useState<ResumeAnalysis | null>(
    null
  );
    ...
    try {
      if (file.type === "application/pdf") {
        ...
        if (text.trim().length < 50) {
          throw new Error(
            "We couldn't extract enough text from this PDF. It might be an image-only scan."
          );
        }
        setResumeText(text);
    ...
    <div className="min-h-screen flex flex-col">
          ...
          <div className="max-w-3xl mx-auto">
            <div className="text-center mb-12">
              ...
              <p className="text-xl text-gray-600">
                Optimize your resume for ATS, identify skill gaps, and get a
            ...
            <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-8 space-y-6">
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-wide flex justify-between">
                  Resume Content
                  {fileProcessing && (
                    <span className="text-blue-600 animate-pulse lowercase font-medium">
                      Extracting text...
                    </span>
                  )}
                </label>
                <div className="relative">
                  ...
                  <div className="absolute bottom-4 right-4 flex gap-2">
                    <label
                      className={`bg-white border border-gray-200 text-gray-600 px-4 py-2 rounded-xl text-sm font-medium cursor-pointer hover:bg-gray-50 transition-colors shadow-sm flex items-center gap-2 ${
      ...
      <footer className="bg-white border-t border-gray-200 py-8">
        <div className="max-w-7xl mx-auto px-4 text-center text-gray-500 text-sm">
          <p>© {new Date().getFullYear()} Made by Ankan Patra</p>
        </div>
      </footer>
    </div>
  );
